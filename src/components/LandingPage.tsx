...
export const LandingPage: React.FC<LandingPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleGoogleLogin = async () => {
    if (!auth) {
      setError('Firebase não configurado. Verifique suas chaves de API.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      if (err.code === 'auth/unauthorized-domain') {
        setError('Este domínio não está autorizado no Console Firebase. Adicione o seu link do Vercel em Authentication > Settings > Authorized Domains.');
      } else {
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4 pt-4">
                  <button
                    ...
                    className="w-full py-5 bg-black text-white rounded-2xl font-black text-lg flex items-center justify-center gap-3 hover:bg-gray-900 transition-all disabled:opacity-50 shadow-2xl relative overflow-hidden group uppercase tracking-widest italic"
                  >
                  ...
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-[10px] uppercase font-bold text-gray-400">
                      <span className="bg-white px-4 tracking-widest">Ou use sua conta Google</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    onClick={handleGoogleLogin}
                    disabled={loading}
                    className="w-full py-4 bg-white border border-gray-200 text-gray-700 rounded-2xl font-bold flex items-center justify-center gap-3 hover:bg-gray-50 transition-all active:scale-95 shadow-sm"
                  >
                    {loading ? (
